...
@AndroidEntryPoint
class TeamActivitiesFragment : Fragment() {
    private val teamActivitiesViewModel: TeamActivitiesViewModel by viewModels()
    ...
    override fun onCreateView(
        ...
        val root = inflater.inflate(R.layout.fragment_team_activities, container, false)

        //Matomo
        MatomoTracker.setParams(MatomoCategory.TEAM_ACITIVITES, "/team-activities")
        MatomoTracker.initFragment()

        adapter = ArrayAdapter(
            this.requireContext(), android.R.layout.simple_list_item_1, emptyList<String>().toMutableList()
        )
        root.findViewById<ListView>(R.id.team_activities_scroll_view).adapter = adapter
    ...
    private fun updateActivities() {
        teamActivitiesViewModel.fetchNewIndividualActivities(
            resources.getStringArray(R.array.individual_activities_array).toList()
        )
    }

    private fun displayActivities(limit: Int = -1) {
            teamActivitiesViewModel.individualActivities.observe(viewLifecycleOwner, {
                adapter.clear()
